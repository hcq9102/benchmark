...
int hpx_main()
{
    ...
    //auto seq_pol = hpx::execution::par.with(hpx::execution::num_cores(1));
    ...
    std::ofstream fout("par1=seq_rotate_medusa_threads=40_res.csv");

    fout << "threadsNum = " << threadsNum <<'\n';
    fout << "n,i,par,par_SR,par_task_SR \n";
    // ,seq/par, seq/par_SR, seq/par_task_SR
    for (std::size_t i = 10; i <= 28; i++)
    {
        std::size_t n = std::pow(2, i);
        mid = n/8;
        //double SEQ_par1 = test(seq_pol, 10, n, std::false_type{});
        double PAR = rotate_async(par_pol, 10, n, std::false_type{});
        double Par_SR = rotate_async(par_sr_pol, 10, n, std::false_type{});
        double Par_Task_SR = rotate_async(par_task_sr_pol, 10, n, std::true_type{});
